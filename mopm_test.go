...
func TestReadPackageFromFile(t *testing.T) {
	dir, _ := os.Getwd()
	got, err := readPackageFromFile(dir + "/sample.mopm.yaml")
	if err != nil {
		t.Errorf("readPackageFromFile() return error: %s, want nil", err)
	}
	if got.Name != "sample" {
		t.Errorf("readPackageFromFile() return got.Name = %s, want 'sample'", got.Name)
	}
	if got.Url != "https://github.com/basd4g/mopm" {
		t.Errorf("readPackageFromFile() return got.Url = %s, want 'https://github.com/basd4g/mopm'", got.Url)
	}
	if got.Description != "This is sample package definition script. It cannot be installed." {
		t.Errorf("readPackageFromFile() return got.Description = %s, want 'This is sample package definition script. It cannot be installed.'", got.Description)
	}
	if got.Environments[0].Architecture != "amd64" {
		t.Errorf("readPackageFromFile() return got.Environments[0].Architecture = %s, wants 'amd64'", got.Environments[0].Architecture)
	}
	if got.Environments[0].Platform != "darwin" {
		t.Errorf("readPackageFromFile() return got.Environments[0].Platform = %s, wants 'darwin'", got.Environments[0].Platform)
	}
	if got.Environments[0].Dependencies != nil {
		t.Errorf("readPackageFromFile() return got.Environments[0].Dependencies = %s, nil", got.Environments[0].Dependencies)
	}
	if got.Environments[0].Verification != "false && false" {
		t.Errorf("readPackageFromFile() return got.Environments[0].Verification = %s, wants 'false && false'", got.Environments[0].Verification)
	}
	if got.Environments[0].Privilege != false {
		t.Errorf("readPackageFromFile() return got.Environments[0].Privilege = true, wants false")
	}
	if got.Environments[0].Script != "echo \"This is sample install script. It is no excution anyware.\"\n" {
		t.Errorf("readPackageFromFile() return got.Environments[0].Script = %s, wants echo \"This is sample install script. It is no excution anyware.\"\n", got.Environments[0].Script)
	}
	if got.Environments[1].Architecture != "amd64" {
		t.Errorf("readPackageFromFile() return got.Environments[1].Architecture = %s, wants 'amd64'", got.Environments[1].Architecture)
	}
	if got.Environments[1].Platform != "linux/ubuntu" {
		t.Errorf("readPackageFromFile() return got.Environments[1].Platform = %s, wants 'linux/ubuntu'", got.Environments[1].Platform)
	}
	if got.Environments[1].Dependencies != nil {
		t.Errorf("readPackageFromFile() return got.Environments[1].Dependencies = %s, nil", got.Environments[1].Dependencies)
	}
	if got.Environments[1].Verification != "false && false" {
		t.Errorf("readPackageFromFile() return got.Environments[1].Verification = %s, wants 'false && false'", got.Environments[1].Verification)
	}
	if got.Environments[1].Privilege != true {
		t.Errorf("readPackageFromFile() return got.Environments[1].Privilege = false, wants true")
	}
	if got.Environments[1].Script != "echo \"This is sample install script. It is no excution anyware.\"\n" {
		t.Errorf("readPackageFromFile() return got.Environments[1].Script = %s, wants echo \"This is sample install script. It is no excution anyware.\"\n", got.Environments[1].Script)
	}
}
